...
%% ===== option == %%
fileName = 'data_sim_04_noise_line.mat';
...
%x_func = [cos(tsym)-1 ;  sin(tsym); 0;0;0;0 ];
...
PointCloud.num = 1000;
PointCloud.range = [6,6,8];
PointCloud.center = [-1,0,0];
...
globalPoints = generate3DPointCloud(PointCloud.range,PointCloud.center,PointCloud.num);

figure(1);
subplot(1,2,1);
scatter3(globalPoints(:,1),globalPoints(:,2),globalPoints(:,3),'MarkerEdgeColor','k','MarkerFaceColor',[0 .75 .75]);
 hold on;
view(-40,10);
axis equal;
...
for i = 1:size(t,2)
    ...
    if mod(i,freq_ratio) == 0
        ...
        % Apply noise
        n_im =  noise.sigma_im*randn(2,PointCloud.num);
        Pixel =Pixel + n_im;
        ...
        observedPoints = [observedPoints;
                          index];
        % Record ideal points
        Cam_z_i = -1*ones(2,PointCloud.num);
